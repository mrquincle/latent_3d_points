	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		...
		for (int j=7;j>=-2;j--){
			...
			for (int k0=0;k0<n;k0+=blockDim.x){
				...
				if (k<n){
					x1=xyz1[i*n*3+k*3+0] - offset1[i*n+3+k*3+0];
					y1=xyz1[i*n*3+k*3+1] - offset1[i*n+3+k*3+1];
					z1=xyz1[i*n*3+k*3+2] - offset1[i*n+3+k*3+2];
				}
				float suml=1e-9f;
				for (int l0=0;l0<m;l0+=Block){
					int lend=min(m,l0+Block)-l0;
					for (int l=threadIdx.x;l<lend;l+=blockDim.x){
						float x2=xyz2[i*m*3+l0*3+l*3+0] - offset2[i*m+3+l0*3+l*3+0];
						float y2=xyz2[i*m*3+l0*3+l*3+1] - offset2[i*m+3+l0*3+l*3+1];
						float z2=xyz2[i*m*3+l0*3+l*3+2] - offset2[i*m+3+l0*3+l*3+2];
						buf[l*4+0]=x2;
						buf[l*4+1]=y2;
						buf[l*4+2]=z2;
			...
			for (int l0=0;l0<m;l0+=blockDim.x){
				...
				if (l<m){
					x2=xyz2[i*m*3+l*3+0] - offset2[i*m*3+l*3+0];
					y2=xyz2[i*m*3+l*3+1] - offset2[i*m*3+l*3+1];
					z2=xyz2[i*m*3+l*3+2] - offset2[i*m*3+l*3+2];
				}
				float sumr=0;
				for (int k0=0;k0<n;k0+=Block){
					int kend=min(n,k0+Block)-k0;
					for (int k=threadIdx.x;k<kend;k+=blockDim.x){
						buf[k*4+0]=xyz1[i*n*3+k0*3+k*3+0] - offset1[i*n*3+k0*3+k*3+0];
						buf[k*4+1]=xyz1[i*n*3+k0*3+k*3+1] - offset1[i*n*3+k0*3+k*3+1];
						buf[k*4+2]=xyz1[i*n*3+k0*3+k*3+2] - offset1[i*n*3+k0*3+k*3+2];
			...
			for (int k0=0;k0<n;k0+=blockDim.x){
				...
				for (int l0=0;l0<m;l0+=Block){
					int lend=min(m,l0+Block)-l0;
					for (int l=threadIdx.x;l<lend;l+=blockDim.x){
						buf[l*4+0]=xyz2[i*m*3+l0*3+l*3+0] - offset2[i*m*3+l0*3+l*3+0];
						buf[l*4+1]=xyz2[i*m*3+l0*3+l*3+1] - offset2[i*m*3+l0*3+l*3+1];
						buf[l*4+2]=xyz2[i*m*3+l0*3+l*3+2] - offset2[i*m*3+l0*3+l*3+2];
		...
		for (int k0=0;k0<n;k0+=blockDim.x){
			...
			for (int l0=0;l0<m;l0+=Block){
				...
				for (int l=threadIdx.x;l<lend*3;l+=blockDim.x)
					buf[l]=xyz2[i*m*3+l0*3+l] - offset2[i*m*3+l0*3+l];
				__syncthreads();
				if (k<n){
					for (int l=0;l<lend;l++){
						float x2=buf[l*3+0];
...
__global__ void multiemdcostgrad2(int b,int n,int m,const float * __restrict__ xyz1,const float * __restrict__ xyz2,
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		...
		for (int k=kbeg;k<kend;k++){
			float x2=xyz2[(i*m+k)*3+0] - offset2[(i*m+k)*3+0];
			float y2=xyz2[(i*m+k)*3+1] - offset2[(i*m+k)*3+1];
			float z2=xyz2[(i*m+k)*3+2] - offset2[(i*m+k)*3+2];
			float subsumx=0,subsumy=0,subsumz=0;
			for (int j=threadIdx.x;j<n;j+=blockDim.x){
				float x1=x2-(xyz1[(i*n+j)*3+0] - offset1[(i*n+j)*3+0]);
				float y1=y2-(xyz1[(i*n+j)*3+1] - offset1[(i*n+j)*3+1]);
				float z1=z2-(xyz1[(i*n+j)*3+2] - offset1[(i*n+j)*3+2]);
...
__global__ void multiemdcostgrad1(int b,int n,int m,const float * __restrict__ xyz1,const float * __restrict__ xyz2,
	...
	for (int i=blockIdx.x;i<b;i+=gridDim.x){
		for (int l=threadIdx.x;l<n;l+=blockDim.x){
			float x1=xyz1[i*n*3+l*3+0] - offset1[i*n*3+l*3+0];
			float y1=xyz1[i*n*3+l*3+1] - offset1[i*n*3+l*3+1];
			float z1=xyz1[i*n*3+l*3+2] - offset1[i*n*3+l*3+2];
			float dx=0,dy=0,dz=0;
			for (int k=0;k<m;k++){
				float x2=xyz2[i*m*3+k*3+0] - offset2[i*m*3+k*3+0];
				float y2=xyz2[i*m*3+k*3+1] - offset2[i*m*3+k*3+1];
				float z2=xyz2[i*m*3+k*3+2] - offset2[i*m*3+k*3+2];
